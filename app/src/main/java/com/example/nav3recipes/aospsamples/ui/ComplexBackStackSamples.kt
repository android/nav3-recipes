...
fun MultipleBackStackSample() {
    ...
    if (homeBackStack.isEmpty()) homeBackStack.add(Home)
    val homeDecorators =
    ...
    val homeEntryProvider =
        entryProvider<NavKey> {
            entry<Home> {
                HomeScreen(backStackString = homeBackStack.map { (it as SampleKey).name }.toString()) {
                    homeBackStack.add(Detail("in Home Tab"))
                }
            }
            entry<Detail> { key ->
                Column(Modifier.fillMaxSize().then(Modifier.padding(8.dp))) {
                    Text(text = "Detail Page ${key.sourceTab}")
                    Text(text = "current backStack:${"${homeBackStack.map { (it as SampleKey).name }}"}")
    ...
    val backStackString1 = userBackStack.map { (it as SampleKey).name }.toString()
    if (userBackStack.isEmpty()) userBackStack.add(User)
    val userDecorators =
        listOf<NavEntryDecorator<NavKey>>(
            rememberSaveableStateHolderNavEntryDecorator(),
            rememberViewModelStoreNavEntryDecorator(),
    ...
    val userEntryProvider =
        entryProvider<NavKey> {
            entry<User> {
                UserScreen(backStackString = backStackString1) {
                    userBackStack.add(Detail("in User Tab"))
                }
            }
            entry<Detail> { key ->
                Column(Modifier.fillMaxSize().then(Modifier.padding(8.dp))) {
                    Text(text = "Detail Page ${key.sourceTab}")
                    Text(text = "current backStack:$backStackString1")
                    var count by rememberSaveable {
                        mutableIntStateOf(0)
                    }
                    Button(onClick = { count++ }){
    ...
    val userTabEntries =
        rememberDecoratedNavEntries(
            ...
            entryDecorators = userDecorators,
            entryProvider = userEntryProvider
        )
