...
internal class DeepLinkTarget(
     ...
     */
    fun <T : NavRecipeKey> match(deepLinkCandidate: DeepLinkCandidate<T>): DeepLinkMatchResult<T>? {
        if (pathSegments.size != deepLinkCandidate.pathSegments.size) return null
        // exact match (url does not contain any arguments)
        if (uri == deepLinkCandidate.uriPattern)
            return DeepLinkMatchResult(deepLinkCandidate.serializer, mapOf())
        ...
        pathSegments
            ...
            .zip(deepLinkCandidate.pathSegments.asSequence())
            .forEach { it ->
        ...
        queries.forEach { query ->
            val name = query.key
            val queryStringParser = deepLinkCandidate.queryValueParsers[name]
        ...
        return DeepLinkMatchResult(deepLinkCandidate.serializer, args)
    }
}
 ...
 *
 * One [DeepLinkCandidate] should be created for each supported deeplink. This means if [T]
 * supports two deeplink patterns:
 * ```
 *  val deeplink1 = www.nav3recipes.com/home
 *  val deeplink2 = www.nav3recipes.com/profile/{userId}
 *  ```
 * Then two [DeepLinkCandidate] should be created
 * ```
 ...
 */
internal class DeepLinkCandidate<T : NavRecipeKey>(
